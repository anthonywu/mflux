...
.PHONY: lint
lint:
	# 🏗️ Running linters (ruff $(RUFF_VERSION), pinned via pyproject.toml), your files will not be mutated.
...
.PHONY: format
format:
	# 🏗️ Running formatter (ruff $(RUFF_VERSION), pinned via pyproject.toml), your files will be changed to comply to formatting configs.
	$(RUFF) format
...
.PHONY: check
check:	# 🏗️ Running pre-commit linter and formatters on files...
