# Vendored from mlx-lm (MIT, Copyright Apple Inc.), stripped of sharding and KV-cache paths.
...
from mflux.models.common_models.gpt_oss.base_utils import (
    BaseModelArgs,
    create_attention_mask,
    scaled_dot_product_attention,
)
from mflux.models.common_models.gpt_oss.rope_utils import initialize_rope
...
class AttentionBlock(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.o_proj = nn.Linear(self.head_dim * config.num_attention_heads, config.hidden_size, bias=True)
    ...
    def __call__(self, x: mx.array, mask: mx.array, cache=None) -> mx.array:
        ...
        v_hat = scaled_dot_product_attention(q, k, v, cache, self.sm_scale, mask=mask, sinks=self.sinks)
    ...
    def sanitize(self, weights):
        ...
        for k, v in weights.items():
            if "gate_up_proj" in k and "bias" not in k:
                ...
                new_weights[k.replace("gate_up_proj_bias", "gate_proj.bias")] = mx.contiguous(v[..., ::2])
                new_weights[k.replace("gate_up_proj_bias", "up_proj.bias")] = mx.contiguous(v[..., 1::2])
